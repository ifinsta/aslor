...
class ProviderEntry:
    ...
    def to_catalog_dict(self) -> dict[str, Any]:
        return {
            ...
            "thinking_enabled": self.thinking_enabled,
            "thinking_budget_tokens": self.thinking_budget_tokens,
            "models": [m.to_model_dict() for m in self.models],
        }
...
class ModelRegistry:
    ...
    def get_provider_for_model(self, model_name: str) -> ProviderEntry | None:
        """Return the :class:`ProviderEntry` that owns *model_name*."""
        entry = self.find_model(model_name)
        if entry is None:
            return None
        for p in self._providers.values():
            if entry in p.models:
                return p
        return None
