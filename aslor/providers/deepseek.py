...
``deepseek-reasoner`` (DeepSeek-R1) returns ``reasoning_content`` on assistant
messages and requires it to be passed back verbatim on the next turn.

The streaming protocol sends reasoning in a separate delta field:
  ``delta.reasoning_content`` — the in-progress chain of thought.
  ``delta.content``           — the visible answer.
"""
...
class DeepSeekAdapter(ProviderAdapter):
    def __init__(self, base_url: str = "https://api.deepseek.com/v1") -> None:
